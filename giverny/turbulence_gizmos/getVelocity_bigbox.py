...
def getVelocity_process_bigbox(npoints,points,dx,min_node_x,min_node_y,min_node_z,max_node_x,max_node_y,max_node_z,cube,
...
# process intepolation    
    if var == 'pr':
        results=np.zeros((npoints,1), dtype=np.float64)
    elif var == 'vel':
        results=np.zeros((npoints,3), dtype=np.float64)
        
    start_time_Lag4 = time.perf_counter()    
        
    for n in range(0,npoints):
        node_x = int(math.floor(points[n,0]/dx)) 
        node_y = int(math.floor(points[n,1]/dx)) 
        node_z = int(math.floor(points[n,2]/dx)) 
...
#        print(node_x,node_y,node_z) 
        ...
        #dx',dy',dz'
        dx_prime = points[n,0]/dx - int(math.floor(points[n,0]/dx))
        dy_prime = points[n,1]/dx - int(math.floor(points[n,1]/dx))
        dz_prime = points[n,2]/dx - int(math.floor(points[n,2]/dx))
...
#    print('output_box lengh',len(output_data[0,0,:]))
        ...
        gx = np.array([lagInt_x[0][0], lagInt_x[1][0], lagInt_x[2][0], lagInt_x[3][0]])
        gy = np.array([lagInt_y[0][0], lagInt_y[1][0], lagInt_y[2][0], lagInt_y[3][0]])
        gz = np.array([lagInt_z[0][0], lagInt_z[1][0], lagInt_z[2][0], lagInt_z[3][0]])
#    print(Fore.RED + 'gx')
#    print(Fore.BLACK + '')
#    print(gx,gx.shape)
...
#    print(Fore.BLACK + '')
#    print(var) 
#        B=output_data[0:4,0:4,0:4]
#        print(Fore.RED + 'B')
#        print(Fore.BLACK + '')
#        print(B,B.shape) 
    
        if var == 'pr':
            A=output_data[(node_z-min_node_z):(4+node_z-min_node_z),(node_y-min_node_y):(4+node_y-min_node_y),(node_x-min_node_x):(4+node_x-min_node_x)]
#            print(Fore.RED + 'A')
#            print(Fore.BLACK + '')
#            print(A,A.shape)  
#            print('node_z-min_node_z',node_z-min_node_z)
#            print('node_y-min_node_y',node_y-min_node_y)
#            print('node_x-min_node_x',node_x-min_node_x)
            sum = np.einsum('ijk,ijkl->l',gk,A) 
#            print(sum)
        elif var == 'vel':
            sum=[]
            A=output_data[(node_z-min_node_z):(4+node_z-min_node_z),(node_y-min_node_y):(4+node_y-min_node_y),(node_x-min_node_x):(4+node_x-min_node_x),:]
...
#            print(A,A.shape)  
            sum = np.einsum('ijk,ijkl->l',gk,A) 
#            print(sum)
#            A=output_data[(node_z-min_node_z):(4+node_z-min_node_z),(node_y-min_node_y):(4+node_y-min_node_y),(node_x-min_node_x):(4+node_x-min_node_x),1:1]
#            sum_v = np.einsum('ijk,ijkl->l',gk,A) 
#            A=output_data[(node_z-min_node_z):(4+node_z-min_node_z),(node_y-min_node_y):#(4+node_y-min_node_y),(node_x-min_node_x):(4+node_x-min_node_x),2:2]        
#            sum_w = np.einsum('ijk,ijkl->l',gk,A) 
#            sum[n]=[sum_u, sum_v, sum_w]
#            print(sum)
#            print('yes go if statement velocity')
#            for i in range(2,6):
#                for j in range(2,6):
#                    for k in range(2,6):
#                        sum_u = sum_u + output_data[k+node_z-min_node_z,j+node_y-min_node_y,i+node_x-min_node_x,0]*lagInt_x[i-2]*lagInt_y[j-2]*lagInt_z[k-2]
#                        sum_v = sum_v + output_data[k+node_z-min_node_z,j+node_y-min_node_y,i+node_x-min_node_x,1]*lagInt_x[i-2]*lagInt_y[j-2]*lagInt_z[k-2]
#                        sum_w = sum_w + output_data[k+node_z-min_node_z,j+node_y-min_node_y,i+node_x-min_node_x,2]*lagInt_x[i-2]*lagInt_y[j-2]*lagInt_z[k-2]
#                        print('node_z-k,node_y-j,node_x-i',node_z-k,node_y-j,node_x-i)
#            results[n]=[sum_u, sum_v, sum_w]
...
#                for j in range(2,6):
#                    for k in range(2,6):
#                        sum = sum + output_data[k+node_z-min_node_z,j+node_y-min_node_y,i+node_x-min_node_x]*lagInt_x[i-2]*lagInt_y[j-2]*lagInt_z[k-2]  
#            results[n]=sum
#        elif var == 'vel':
#            sum_u=0
#            sum_v=0
...
#            results[n]=[sum_u, sum_v, sum_w]
    ...
    print(f'\nstep 4th order at one point took = {round(end_time_Lag4 - start_time_Lag4, 3)} seconds ({round((end_time_cutout_onevoxel - start_time_cutout_onevoxel) / 60, 3)} minutes)')  
