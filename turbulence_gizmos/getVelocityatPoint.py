...
#    print(Fore.RED + 'output_data[:,:,:,0]')
#    print(Fore.BLACK + '')
#    print('output_data[:,:,:,0',output_data[:,:,:,0],output_data[:,:,:,0].shape)     
    
    #Calculate inteporlation
    if var == 'pr':
        sum = np.einsum('ijk,ijkl->l',gk,output_data) 
#        sum=0
...
#                    sum = sum + output_data[k,j,i]*lagInt_x[i-2]*lagInt_y[j-2]*lagInt_z[k-2]  
        ...
        sum = np.einsum('ijk,ijkl->l',gk,output_data) 
#        sum_v = np.einsum('ijk,ijkl->l',gk,output_data[:,:,:,1]) 
#        sum_w = np.einsum('ijk,ijkl->l',gk,output_data[:,:,:,2]) 
#        sum_u=0
#        sum_v=0
#        sum_w=0
